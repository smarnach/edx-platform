...
class CoursesApiTests(TestCase):
    ...
    def setUp(self):
        self.test_server_prefix = 'http://testserver'
    ...
    def do_post(self, uri, data):
        ...
        response = self.client.post(uri, content_type='application/json', data=json_data, **HEADERS)
        return response

    def do_delete(self, uri):
        """Submit an HTTP DELETE request"""
        response = self.client.delete(uri, content_type='application/json', **HEADERS)
        return response
    ...
    def test_courses_list_get_with_filter(self):
        ...
        for course in response.data['results']:
            ...
            if course['id'] == self.test_course_id:
                self.assertValidResponseCourse(course, self.course)
                self.assertIsNotNone(course['image_url'])
...
COURSE = CourseFactory.create(display_name=COURSE_NAME, raw_grader=[
    {
        "min_count": 24,
        "weight": 0.2,
        "type": "Homework",
...
class CourseViewTestsMixin(object):
    ...
    def test_get_invalid_course(self):
        ...
        """
        response = self.client.get(reverse(self.view, kwargs={'course_id': INVALID_COURSE_ID}), **HEADERS)
        self.assertEqual(response.status_code, 404)
    ...
    def test_get(self):
        ...
        """
        response = self.client.get(reverse(self.view, kwargs={'course_id': COURSE_ID}), **HEADERS)
        self.assertEqual(response.status_code, 200)
...
@override_settings(EDX_API_KEY=TEST_API_KEY)
class CourseAssignmentsTests(CourseViewTestsMixin, TestCase):
...
@override_settings(EDX_API_KEY=TEST_API_KEY)
class CourseGradingPolicyTests(CourseViewTestsMixin, TestCase):
